...
const ROTULO_NIVEL = { completo: 'COMPLETO', parcial: 'FALTA POUCO', critico: 'FALTA MUITO' } as const;
      ...
      <Section title="Cronograma físico — em que pé está a obra">
        <KpiGrid>
          ...
          <KpiCard
            ...
            sub={obra.valor_contrato != null ? `Contrato: ${formatBRL(obra.valor_contrato)}` : 'sem contrato'}
          />
          <KpiCard
            label="Prazo usado"
        ...
        {cronograma.etapas.length === 0 ? (
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              ...
              <Text style={[styles.tableHeaderCell, { flex: 2, textAlign: 'right' }]}>Concluído</Text>
            ...
            {cronograma.etapas.map((e, i) => (
              <View key={e.id} style={[styles.tableRow, i % 2 === 1 ? styles.tableRowAlt : {}]}>
                ...
                <Text style={[styles.tableCell, { flex: 2 }, styles.cellMuted]}>{formatDate(e.medido_em)}</Text>
                <Text style={[styles.tableCell, { flex: 2 }, styles.cellMuted]}>{formatDate(e.data_prevista)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {cronograma.orcado.linhas.length === 0 ? (
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              ...
              <Text style={[styles.tableHeaderCell, { flex: 2, textAlign: 'right' }]}>Realizado</Text>
            ...
            {cronograma.orcado.linhas.map((l, i) => (
              <View key={l.categoria_id} style={[styles.tableRow, i % 2 === 1 ? styles.tableRowAlt : {}]}>
                <Text style={[styles.tableCell, { flex: 4 }]}>{l.categoria}</Text>
                <Text style={[styles.tableCell, { flex: 2 }, styles.cellRight]}>{formatBRL(l.orcado)}</Text>
                <Text style={[styles.tableCell, { flex: 2 }, styles.cellRight]}>{formatBRL(l.realizado)}</Text>
                <Text
                  style={[
                    styles.tableCell,
                    { flex: 2 },
